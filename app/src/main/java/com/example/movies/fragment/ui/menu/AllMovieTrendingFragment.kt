...
import com.example.movies.databinding.FragmentAllMovieTrendingBinding
...
class AllMovieTrendingFragment : Fragment() {
    private lateinit var binding: FragmentAllMovieTrendingBinding
    private var allMovieTrendingAdapter: AdapterMovie? = null
    ...
    override fun onCreateView(
        ...
        binding = FragmentAllMovieTrendingBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        allMovieTrendingAdapter = AdapterMovie(requireContext())
        binding.recyclerAllTrending.apply {
            layoutManager = GridLayoutManager(requireContext(), 2)
            adapter = allMovieTrendingAdapter
        }

        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.nowAllPlayingMovie.collect { allTrending ->
                allMovieTrendingAdapter?.submitList(allTrending)
                onClickIem()
            }
        }

        binding.btnBackNowPlay.setOnClickListener {
            findNavController().navigate(R.id.action_allMovieTrendingFragment_to_navGraphFragment)
        }

        binding.btnFilterNowPlay.setOnClickListener {
            val action =
                AllMovieTrendingFragmentDirections.actionAllMovieTrendingFragmentToFilterFragment("Trending Filter")
            findNavController().navigate(action)
        }
        observeAndSortTrending(viewModel, filterViewModel)
    }

    private fun observeAndSortTrending(
        allMovieTrendingViewModel: HomeViewModel,
        filterViewModel: FilterViewModel
    ) {
        viewLifecycleOwner.lifecycleScope.launch {
            combine(
                allMovieTrendingViewModel.allTrendingMovie,
                filterViewModel.sortState
            ) { allMovieTrendingList, sortState ->
                val mappedList = allMovieTrendingList.sortedByDescending { it.name?.lowercase() }
                ...
                allMovieTrendingAdapter?.submitList(sortedList)
                onClickIem()
            }
        }
    }
    ...
    private fun sentDataDetail(data: MovieData) {
        ...
        findNavController().navigate(R.id.action_allMovieTrendingFragment_to_detailFragment, bundle)
    }

    private fun onClickIem() {
        allMovieTrendingAdapter?.onClick = { movie ->
